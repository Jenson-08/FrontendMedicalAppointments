...
function row(listado, cita) {
    var tr = $("<tr />");
    tr.html(`<td>${cita.fecha}</td>
                 <td>${cita.titulocita}</td>
                 <td>${cita.nomPaciente}</td>
                 <td>${cita.horacita}</td>
                 <td>${cita.id_medico}</td>
                 <td id="atendercita"><img src="http://localhost:8080/TareaCrudJs/images/edit.png"></td>                                            `);
  
    tr.find("#atendercita").on("click", () => {
        retornacitaporNombre(cita.titulocita);
    });
    listado.append(tr);
}
...
function retornapersonaporId2(cedula) {
    ...
    function fetchAndList() {
        const request = new Request(backend + '/citas/'+doctor.id, {method: 'GET', headers: {}});
        (async () => {
            try {
                const response = await fetch(request);
                if (!response.ok) {
